...
Public Class frmPenjualan
    ...
    Private Sub ComboBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox1.SelectedIndexChanged
        Call KONEKSI()
        CMD = New OdbcCommand("select * from pelanggan where id= '" & ComboBox1.Text & "'", CONN)
        DR = CMD.ExecuteReader
        If DR.HasRows Then
            lblNamaPlg.Text = DR!nama
    ...
    Sub NomorOtomatis()
        Call KONEKSI()
        CMD = New OdbcCommand("select * from penjualan where NoJual in (select max(NoJual) from penjualan)", CONN)
    ...
    Private Sub TextBox1_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TextBox1.KeyPress
        If e.KeyChar = Chr(13) Then
            If Val(TextBox1.Text) < Val(Label16.Text) Then
                MsgBox("Pembayaran kurang nih...", vbInformation, "Error")
            ElseIf Val(TextBox1.Text) = Val(Label16.Text) Then
                lblKembali.Text = 0
            ElseIf Val(TextBox1.Text) > Val(Label16.Text) Then
                lblKembali.Text = Val(TextBox1.Text) - Val(Label16.Text)
                Button1.Focus()
            End If
        End If
    End Sub
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If lblKembali.Text = "" Or lblNamaPlg.Text = "" Or Label16.Text = 0 Then
            MsgBox("Transaksi tidak ada, silahkan melakukan transaksi terlebih dahulu!", vbInformation, "Info")
        Else
            TglMySQL = Format(Today, "yyyy-MM-dd")
            Dim SimpanJual As String = "insert into penjualan values ('" & lblNoJual.Text & "', '" & TglMySQL & "', '" & lblJam.Text & "', '" & lblItem.Text & "', '" & Label16.Text & "', '" & TextBox1.Text & "', '" & lblKembali.Text & "', '" & ComboBox1.Text & "', '" & frmMenu.Status1.Text & "')"
            CMD = New OdbcCommand(SimpanJual, CONN)
            CMD.ExecuteNonQuery()

            For Baris As Integer = 0 To DataGridView1.Rows.Count - 2
                Dim SimpanDetail As String = "insert into detailjual values ('" & lblNoJual.Text & "',  '" & DataGridView1.Rows(Baris).Cells(0).Value & "', '" & DataGridView1.Rows(Baris).Cells(1).Value & "', '" & DataGridView1.Rows(Baris).Cells(2).Value & "', '" & DataGridView1.Rows(Baris).Cells(3).Value & "', '" & DataGridView1.Rows(Baris).Cells(4).Value & "')"
                ...
                DR = CMD.ExecuteReader
                DR.Read()
                Dim KurangiStok As String = "Update barang set jumlah= '" & DR.Item("jumlah") - DataGridView1.Rows(Baris).Cells(3).Value & "' where id= '" & DataGridView1.Rows(Baris).Cells(0).Value & "'"
