...
Public Class frmHistoriPembelian
    Sub KondisiAwal()
        ComboBox1.Text = ""
        lblTanggal.Text = ""
        lblTotalHarga.Text = ""
    ...
    Sub MunculKodePelanggan()
        ...
        CMD = New OdbcCommand("select * from pembelian", CONN)
    ...
    Private Sub ComboBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox1.SelectedIndexChanged
        Call KONEKSI()
        CMD = New OdbcCommand("select * from pembelian where NoBeli= '" & ComboBox1.Text & "'", CONN)
        DR = CMD.ExecuteReader
        DR.Read()
        If DR.HasRows Then
            lblTanggal.Text = Format(DR.Item("tglBeli"), "dd-MMMM-yyyy")
            lblTotalHarga.Text = DR.Item("totalbeli")
            txtDibayar.Text = DR.Item("dibayar")
            lblKembali.Text = DR.Item("kembali")
        End If
        Call KONEKSI()
        DA = New OdbcDataAdapter("select namaBrg, hrgBeli, jmlBeli, subTotal from detailbeli where NoBeli = '" & ComboBox1.Text & "'", CONN)
        DS = New DataSet
        DA.Fill(DS)
    ...
    Private Sub btnCetak_Click(sender As Object, e As EventArgs) Handles btnCetak.Click
        If ComboBox1.Text = "" Then
            MsgBox("Ada yang belum terisi nih...", vbInformation, "Error")
        Else
            cetak.Show()
            cetak.CRV.SelectionFormula = "totext({pembelian.noBeli})='" & ComboBox1.Text & "'"
            cetak.CRV.ReportSource = "notaBeli.rpt"
